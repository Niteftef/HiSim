...
class HeatPumpHplibController(Component):
    ...
    def conditions_heating_cooling_off(
        ...
        cooling_set_temperature = set_heating_flow_temperature_in_celsius # Todo: this looks wrong, but maybe calculation is correct here, then probably error is in heat distribution sysstem

        if self.controller_heatpumpmode == "heating":
            if (
                water_temperature_input_in_celsius
                >= heating_set_temperature + storage_temperature_modifier  # Todo: does temperaturemodifier leads tooverheating of HDS?
                or summer_heating_mode == "off"
            ):
                self.controller_heatpumpmode = "off"
                return
            ...
            if (
                water_temperature_input_in_celsius > (cooling_set_temperature + 0.5)
