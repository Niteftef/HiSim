...
def test_gas_heater():
    ...
    control_signal_channel = cp.ComponentOutput("FakeControlSignal",
                             "ControlSignal",
                                        lt.LoadTypes.ANY,
                                        lt.Units.PERCENT)

    mass_flow_input_temperature_channel = cp.ComponentOutput( "FakeMassflowInputTemperature",
                              "MassflowInputTemperature",
                                                      lt.LoadTypes.WATER,
                                                      lt.Units.CELSIUS)

    number_of_outputs = fft.get_number_of_outputs([control_signal_channel,mass_flow_input_temperature_channel,my_gas_heater])
    stsv: cp.SingleTimeStepValues = cp.SingleTimeStepValues(number_of_outputs)

    # Link inputs and outputs
    my_gas_heater.control_signal_channel.source_output = control_signal_channel
    my_gas_heater.mass_flow_input_tempertaure_channel.source_output = mass_flow_input_temperature_channel

    # Add Global Index and set values for fake Inputs
    fft.add_global_index_of_components([control_signal_channel,mass_flow_input_temperature_channel,my_gas_heater])
    ...
    assert stsv.values[my_gas_heater.mass_flow_output_channel.global_index] == 0.2582496413199426
    # Temperature of Water out of GasHeater
    assert stsv.values[my_gas_heater.mass_flow_output_temperature_channel.global_index] == temperature_delta_in_celsius + stsv.values[mass_flow_input_temperature_channel.global_index]
